...
(function (CreateAssertStringifyTransformer) {
    function transform(project, modulo, expression) {
        if (!expression.typeArguments || !expression.typeArguments[0])
            throw new Error("Error on TSON.assertStringify(): generic argument is not specified.");
        var type = project.checker.getTypeFromTypeNode(expression.typeArguments[0]);
        if (type.isTypeParameter())
            throw new Error("Error on TSON.assertStringify(): non-specified generic argument.");
